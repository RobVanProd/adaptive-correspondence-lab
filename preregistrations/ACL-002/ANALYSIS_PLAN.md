...
| strict-confirmatory epsilons | `1e-4, 3e-4, 1e-3` |
| non-gating extended-local epsilons | `3e-3, 1e-2` |
| stress epsilons | `3e-2, 1e-1` |
| inherited exact-correspondence tolerance | `2e-14` |
| row-Jacobian mass tolerance | `2e-14` |
| accumulated tangent-mass tolerance | `2e-13` |
| perturbed-simplex absolute tolerance | `5e-13` |
| matrix-oracle maximum absolute tolerance | `5e-13` |
...
3. Fit each remaining source landscape separately through the origin over all three
   strict-confirmatory epsilons.
4. Set `alpha_source` to the Type-7 median of landscape alphas.
5. For each regular target and prediction layer, calculate three relative errors and
   reduce them to their maximum landscape score.
...
prediction, region, stratum, and iterative/matrix-oracle error. No row is dropped.

## Missing, invalid, or exceptional data

The deterministic implementation admits no missing-value policy. A non-finite value,
simplex violation, oracle mismatch, non-clean worktree (including untracked files), lock
mismatch, registry mismatch, duplicate ID, incomplete grid, or unexpected row count
stops execution before summary verdicts. Failures are not imputed, winsorized, clipped,
projected, or silently omitted.
...
10. Extended-local and stress results, labeled non-gating.

The target set is a deterministic held-out benchmark. Median and Q0.90 are descriptive
criteria, not confidence statements about a sampled population. Catalog reuse across
new source/target combinations makes this a within-family transport test.
